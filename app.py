...
# ---------- آماده‌سازی ایندکس ----------
def ensure_index():
    """ایندکس را در صورت نبود یا قدیمی‌بودن (ناهم‌خوان با داده‌ها) بازسازی می‌کند.

    قبلاً فقط وجود فایل بررسی می‌شد؛ به همین دلیل ایندکسِ قدیمیِ داخل مخزن
    (فقط ۴۵ قطعه‌ی پروتکل) هرگز بازسازی نمی‌شد و ۱۱۷۰ قطعه‌ی مقالات/راهنماها
    نادیده گرفته می‌شد.
    """
    if store.manifest_matches():
        log.info("ایندکس آماده و به‌روز پیدا شد.")
        return
    log.info("ایندکس موجود نیست یا نسبت به داده‌ها قدیمی است؛ در حال ساخت...")
